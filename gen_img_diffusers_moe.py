    ...
    if args.network_module:
        ...
        for i, network_module in enumerate(args.network_module):
            ...
            if args.network_weights and i < len(args.network_weights):
                ...
                if 'face' in network_weight:
                    network_face, weights_sd_face = imported_module.create_network_from_weights(
                        network_mul, network_weight, vae, text_encoder, unet, for_inference=True, **net_kwargs
                    )
                if 'hand' in network_weight:
                    network_hand, weights_sd_hand = imported_module.create_network_from_weights(
                        network_mul, network_weight, vae, text_encoder, unet, for_inference=True, **net_kwargs
                    )
            else:
        ...
        if not args.network_merge or not mergeable:
            ...
            network_face.apply_to(text_encoder, unet, apply_to=False)  ## since we load from weight, hence, the extra parameter is not required.
            network_hand.apply_to(text_encoder, unet, apply_to=False)

            info = network_face.load_state_dict(weights_sd_face, False)  # network.load_weightsを使うようにするとよい
            print(f"weights are loaded: {info}")
            info = network_hand.load_state_dict(weights_sd_hand, False)  # network.load_weightsを使うようにするとよい

            if args.opt_channels_last:
                network.to(memory_format=torch.channels_last)

            network_hand.to(dtype).to(device)
            network_face.to(dtype).to(device)
    ...
    for gen_iter in range(args.n_iter):
        ...
        def process_batch(batch: List[BatchData], highres_fix, highres_1st=False):
            ...
            for i, (_, (_, prompt, negative_prompt, seed, init_image, mask_image, clip_prompt, guide_image), _) in enumerate(batch):
                prompts.append(prompt + " best quality, 8K, good face, clear hand")
                negative_prompts.append(negative_prompt + " poor quality, unclear hand, bad face ")
                seeds.append(seed)
...
def setup_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--network_module_moe", type=str, default=None, nargs="*", help="additional network module to use / 追加ネットワークを使う時そのモジュール名"
    )
    parser.add_argument(
        "--network_weights", type=str, default=None, nargs="*", help="additional network weights to load / 追加ネットワークの重み"
    )
    parser.add_argument(
        "--network_weights_moe", type=str, default=None, nargs="*", help="additional network weights to load / 追加ネットワークの重み"
    )

...
class BaseDataset(torch.utils.data.Dataset):
    ...
    def load_image(self, image_path):
        ...
        img = cv2.resize(img, (self.width, self.height), interpolation=cv2.INTER_AREA)  # INTER_AREAでやりたいのでcv2でリサイズ
        return img
...
def replace_unet_cross_attn_to_memory_efficient():
    ...
    ## modified for diffusers higher version instead of 0.10.0
    cross_attention.CrossAttention.forward = forward_flash_attn
...
def replace_unet_cross_attn_to_xformers():
    ...
    ## modified for diffusers higher version instead of 0.10.0
    cross_attention.CrossAttention.forward = forward_xformers
...
def add_training_arguments(parser: argparse.ArgumentParser, support_dreambooth: bool):
    ...
    parser.add_argument("--resume", type=str, default=None, help="saved state to resume training / 学習再開するモデルのstate") # --load_model

    parser.add_argument("--train_batch_size", type=int, default=16, help="batch size for training / 学習時のバッチサイズ")
    parser.add_argument(
        "--max_token_length",
        type=int,
        default=None,
...
def load_tokenizer(args: argparse.Namespace):
    ...
    if tokenizer is None:
        if args.v2:
            ...
            "/home/cxk/xknfs/zhujie/prompt-to-prompt-main/stable-diffusion-v1-5", subfolder="tokenizer", revision=None)
...
def _load_target_model(args: argparse.Namespace, weight_dtype, device="cpu", safetensors=False):
    name_or_path = args.load_model
    ...
    if load_stable_diffusion_format:
        ...
        try:
            pipe = StableDiffusionPipeline.from_pretrained(name_or_path, tokenizer=None, safety_checker=None, use_safetensors=safetensors)
...
def transform_if_model_is_DDP_moe(text_encoder, unet, network_face, network_hand, network_moe):
    # Transform text_encoder, unet and network from DistributedDataParallel
    return (model.module if type(model) == DDP else model for model in [text_encoder, unet, network_face, network_hand, network_moe] if model is not None)


def load_target_model(args, weight_dtype, accelerator, safetensors):
    ...
    for pi in range(accelerator.state.num_processes):
        if pi == accelerator.state.local_process_index:
            ...
            text_encoder, vae, unet, load_stable_diffusion_format = _load_target_model(
                args, weight_dtype, accelerator.device if args.lowram else "cpu", safetensors
            )
...
def sample_images(
    ...
    pipeline.__init__additional__(args.clip_skip)
        ...
        with accelerator.autocast():
            for i, prompt in enumerate(prompts):
                ...
                if isinstance(prompt, dict):
                    ...
                    sample_steps = prompt.get("sample_steps", 50) 
                ...
                ## if need negative_prompt = "poor quality, unclear hand, bad face"

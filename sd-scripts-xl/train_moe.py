...
class MOETrainer:
    ...
    def train(self, args):
        ...
        accelerator.print("import network module:", args.network_module_lora)
        network_module = importlib.import_module(args.network_module_lora)
        accelerator.print("import network module:", args.network_module_moe)
        ...
        if network_face is None:
            return

        # network_has_multiplier = hasattr(network_face, "set_multiplier")
        # network_has_multiplier = hasattr(network_hand, "set_multiplier")
        
        network_has_multiplier = None

        if hasattr(network_face, "prepare_network"):
            network_face.prepare_network(args)
            network_hand.prepare_network(args)

        if args.scale_weight_norms and not hasattr(network_face, "apply_max_norm_regularization"):
            logger.warning(
        ...
        if args.gradient_checkpointing:
            ...
            del t_enc
            network_moe.enable_gradient_checkpointing()  # may have no effect
            network_moe.enable_gradient_checkpointing()  # may have no effect
        ...
        if args.deepspeed:
            ...
            network_moe, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
                network_moe, optimizer, train_dataloader, lr_scheduler
            )
            training_model = network_moe
        ...
        accelerator.unwrap_model(network_moe).prepare_grad_etc(text_encoder, unet)
        ...
        if hasattr(accelerator.unwrap_model(network_moe), "on_step_start"):
            on_step_start = accelerator.unwrap_model(network_moe).on_step_start
        else:
        ...
        for epoch in range(num_train_epochs):
            ...
            accelerator.unwrap_model(network_moe).on_epoch_start(text_encoder, unet)
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                with accelerator.accumulate(training_model):
                    ...
                    if accelerator.sync_gradients:
                        self.all_reduce_network(accelerator, network_moe)  # sync DDP grad manually
                        if args.max_grad_norm != 0.0:
                            params_to_clip = accelerator.unwrap_model(network_moe).get_trainable_params()
                ...
                if args.scale_weight_norms:
                    keys_scaled, mean_norm, maximum_norm = accelerator.unwrap_model(network_moe).apply_max_norm_regularization(
                ...
                if accelerator.sync_gradients:
                    ...
                    if args.save_every_n_steps is not None and global_step % args.save_every_n_steps == 0:
                        ...
                        if accelerator.is_main_process:
                            ...
                            save_model(ckpt_name, accelerator.unwrap_model(network_moe), global_step, epoch)
            ...
            if args.save_every_n_epochs is not None:
                ...
                if is_main_process and saving:
                    ...
                    save_model(ckpt_name, accelerator.unwrap_model(network_moe), global_step, epoch + 1)
        ...
        if is_main_process:
            network_moe = accelerator.unwrap_model(network_moe)
...
def setup_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--face_network_weights", type=str, default=None, help="pretrained weights for network / 学習するネットワークの初期重み"
    )
    parser.add_argument(
        "--hand_network_weights", type=str, default=None, help="pretrained weights for network / 学習するネットワークの初期重み"
    )

...
CLIP_VISION_MODEL = "/root/paddlejob/workspace/env_run/output/CLIP-ViT-bigG-14-laion2B-39B-b160k"
...
def main(args):
    ...
    if args.network_module:
        ...
        if not mergeable or i >= network_merge:
            # not merging
            network_face.apply_to([text_encoder1, text_encoder2], unet, apply_to=False)  ## since we load from weight, hence, the extra parameter is not required.
            network_hand.apply_to([text_encoder1, text_encoder2], unet, apply_to=False)
    ...
    # print("clip_skip: ", args.clip_skip)
    pipe = PipelineLike(
        device,
        vae,
